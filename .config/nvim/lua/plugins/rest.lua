return {
  {
    ...
    cmd = { "CurlOpen" },
    ft = { "curl", "http" }, -- Lazy loads on both .curl and .http files
    dependencies = {
      "nvim-lua/plenary.nvim",
    },
    init = function()
      -- Register .http extension as 'curl' filetype before plugin loads
      vim.filetype.add({
        extension = {
          http = "curl",
        },
    ...
    opts = {
      ...
      global_keymaps = {
        ["Open scratchpad"] = {
          "<leader>rb",
          function()
            vim.cmd("e ~/.local/share/kulala/scratchpad.http")
          end,
